...
\medskip
Package \nmfpack\ can be downloaded from \url{http://web.cbio.uct.ac.za/~renaud/nmf/}.
It loads with the following call:
<<load_library>>=
library(NMF)
@
...
containned in $X$ into $r$ factors: the columns of $W$. Depending on the
application field, these factors are given different names: basis images,
metagenes, source signals.
